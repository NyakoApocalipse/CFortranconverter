...
// If forced reversion occurs, FORTRAN 77 advances to the next record and
// rescans the format, starting with the right-most left parenthesis, including
// any repeat-count indicators. It then re-uses this part of the format. If
// there are no inner parenthesis in the FORMAT statement, then the entire
// format is reused.
...
// specification and another effective input / output list item is not
// specified, format control terminates.However, if another effective input /
// output list item is specified, the file is positioned in a manner identical
// to the way it is positioned when a slash edit descriptor is
// processed(10.6.2).Format control then reverts to the beginning of the format
// item terminated by the last preceding right parenthesis.If there is no such
// preceding right parenthesis, format control reverts to the first left
// parenthesis of the format specification.If any reversion occurs, the reused
// portion of the format specification must contain at least one data edit
// descriptor.If format control reverts to a parenthesis that is preceded by a
// repeat specification, the repeat specification is reused.Reversion of format
// control, of itself, has no effect on the scale factor(10.6.5.1), the sign
// control edit descriptors(10.6.4), or the blank interpretation edit
// descriptors (10.6.6).

_NAMESPACE_FORTRAN_BEGIN
template <typename T, typename F> struct ImpliedDo {
  ...
  bool has_next() const { return _map_impl_has_next(cur, dim, lb, sz); }
  T get_next() const {
...
auto make_implieddo(const fsize_t (&_lb)[D], const fsize_t (&_to)[D],
                    F &&func) {
  typedef typename function_traits<
      std::remove_reference_t<decltype(func)>>::result_type T;
...
template <typename... Types> struct IOStuff {
  IOStuff() {}
...
struct IOFormat {
  ...
  IOFormat(const std::string &s, int rev_start)
      : reversion_start(rev_start), p(0) {
    fmt = s;
    reversion_end = (int)fmt.size();
  }
  IOFormat(const char *ch, int rev_start, int rev_end)
      : reversion_start(rev_start), reversion_end(rev_end), p(0) {
    fmt = std::string(ch);
  }
  IOFormat(const std::string &s, int rev_start, int rev_end)
      : reversion_start(rev_start), reversion_end(rev_end), p(0) {
    fmt = s;
  }
  ...
  const char &operator[](size_t off) const { return fmt[off]; }
  char &operator[](size_t off) { return fmt[off]; }
  size_t size() const { return fmt.size(); }
  std::string substr(size_t offset = 0,
                     size_t count = std::string::npos) const {
    return fmt.substr(offset, count);
  }
  const char *c_str() const { return fmt.c_str(); }
  size_t &index() const {
...
inline void _str_fprintf(FILE *f, const std::string &_format,
                         const std::string &x) {
...
template <typename... Types>
void _forwrite_dispatch(FILE *f, IOFormat &format,
                        const IOStuff<Types...> &iostuff) {
  foreach_tuple(iostuff.tp, [&](auto x) { _forwrite_dispatch(f, format, x); });
};
...
// free format
// write free step 2
inline void _forwritefree_one(FILE *f, float x) { fprintf(f, "%f", x); };
inline void _forwritefree_one(FILE *f, int x) { fprintf(f, "%d", x); };
inline void _forwritefree_one(FILE *f, long long x) { fprintf(f, "%lld", x); };
inline void _forwritefree_one(FILE *f, double x) { fprintf(f, "%lf", x); };
inline void _forwritefree_one(FILE *f, long double x) { fprintf(f, "%Lf", x); };
...
inline void _forwritefree_one(FILE *f, const char *x) { fprintf(f, "%s", x); };
template <typename T> void _forwritefree_one(FILE *f, T x) {
  fprintf(f, "[object %s] %p", typeid(T).name(), &x);
};

template <typename T> void _forwritefree_one_arrf(FILE *f, const farray<T> &x) {
  auto iter = x.cbegin();
  for (fsize_t i = 0; i < x.flatsize(); i++) {
    if (i > 0)
      fprintf(f, "\t");
...
template <typename T> void _forwritefree_dispatch(FILE *f, const farray<T> &x) {
  _forwritefree_one_arrf(f, x);
};
template <typename T> void _forwritefree_dispatch(FILE *f, const T &x) {
...
// write free step 0
inline void forwritefree(FILE *f) { fprintf(f, "\n"); };
template <typename T, typename... Args>
...
template <typename T> void forwritefree(FILE *f, const T &x) {
  _forwritefree_dispatch(f, x);
  fprintf(f, "\n");
};

inline void forprintfree() { forwritefree(stdout, "\n"); };
template <typename T, typename... Args>
...
inline void forprint(IOFormat &format) { forwrite(stdout, format); };
inline void forprint(const std::string &format) { forwrite(stdout, format); };

// read
inline void _forread_noargs(FILE *f, IOFormat &format) { format.strip_front(); }
...
template <typename T> void _forread_one(FILE *f, IOFormat &format, T &x) {
  // strip front
...
template <typename T> void _forread_dispatch(FILE *f, IOFormat &format, T *x) {
...
void _forread_dispatch(FILE *f, IOFormat &format, IOStuff<Types...> &iostuff) {
  foreach_tuple(iostuff.tp, [&](auto &x) { _forread_dispatch(f, x); });
};
template <typename T, typename F>
...
inline void _forreadfree_one(FILE *f, long long &x) { fscanf(f, "%lld", &x); };
inline void _forreadfree_one(FILE *f, double &x) { fscanf(f, "%lf", &x); };
inline void _forreadfree_one(FILE *f, long double &x) { fscanf(f, "%Lf", &x); };
...
inline void _forreadfree_one(FILE *f, char *x) { fscanf(f, "%s", x); };
inline void _forreadfree_one(FILE *f, char &x) { fscanf(f, "%c", &x); };

template <typename T> void _forreadfree_one_arrf(FILE *f, farray<T> &x) {
  auto iter = x.begin();
  for (fsize_t i = 0; i < x.flatsize(); i++) {
...
template <typename T> void _forreadfree_dispatch(FILE *f, farray<T> *x) {
  _forreadfree_one_arrf(f, *x);
};
template <typename T> void _forreadfree_dispatch(FILE *f, T *x) {
  _forreadfree_one(f, *x);
};
template <typename... Types>
void _forreadfree_dispatch(FILE *f, IOStuff<Types...> &iostuff) {
  foreach_tuple(iostuff.tp, [&](auto &x) { _forreadfree_dispatch(f, x); });
};
template <typename T, typename F>
...
// read free step 0
template <typename T> void forreadfree(FILE *f, T &&x) {
...
/* add extension for read from `char *` */
